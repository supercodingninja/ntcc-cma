...
export function A11yProvider({ children }: { children: ReactNode }) {
  ...
  const toggleMode = (m: AccessMode) => {
    setModes((p) => {
      const next = { ...p, [m]: !p[m] };
      if (m === 'highContrast') setHighContrastState(next.highContrast);
      if (m === 'screenReader') setNarrationEnabled(next.screenReader);
      if (m === 'noFlash' && next.noFlash) setReducedMotion(true);
      if (m === 'calm' && next.calm) setReducedMotion(true);
      return next;
    });
  };
