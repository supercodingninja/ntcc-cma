...
// Krumhansl-Schmuckler profiles (published, open research).
...
export async function analyzeAudio(file: File): Promise<KeyGuess | null> {
  ...
  try {
    ...
    // Key: correlate chroma against every rotation of both profiles.
    let bestKey = 'C', bestCorr = -2;
    for (let rot = 0; rot < 12; rot++) {
      const rotated = [...chroma.slice(rot), ...chroma.slice(0, rot)];
      const cMaj = pearson(rotated, MAJOR_PROFILE);
      if (cMaj > bestCorr) { bestCorr = cMaj; bestKey = NOTE_NAMES[rot]; }
      const cMin = pearson(rotated, MINOR_PROFILE);
      if (cMin > bestCorr) { bestCorr = cMin; bestKey = `${NOTE_NAMES[rot]}m`; }
    }
