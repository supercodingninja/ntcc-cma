...
export function listIngestors(): Ingestor[] {
  return [...state.ingestors.values()];
}

export function listRenderers(): Renderer[] {
  return [...state.renderers.values()];
}
...
export async function safeRender<Output>(
  id: string, doc: USMGDocument,
): Promise<{ output?: Output; errors?: string[] }> {
  const r = state.renderers.get(id);
  if (!r) return { errors: [`Renderer "${id}" not registered`] };
  try {
    return (await r.render(doc)) as { output?: Output; errors?: string[] };
  } catch (err) {
    return { errors: [`Renderer "${id}" failed safely: ${String(err)}`] };
  }
}
