...
async function toGray(src: string, maxW = 1200): Promise<{ data: Uint8ClampedArray; width: number; height: number }> {
  const img = new Image();
...
function projection(gray: Uint8ClampedArray, w: number, h: number, threshold = 128): Float32Array {
...
function detectSystems(prof: Float32Array): StaffSystem[] {
  const rows: number[] = [];
  for (let y = 1; y < prof.length - 1; y++) {
    if (prof[y] > 0.45 && prof[y] >= prof[y - 1] && prof[y] >= prof[y + 1]) {
      // merge near-duplicates
      if (rows.length === 0 || y - rows[rows.length - 1] > 2) rows.push(y);
    }
  }
