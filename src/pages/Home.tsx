...
type Tab = 'library' | 'song' | 'setlist' | 'history' | 'director' | 'lighting' | 'engineer' | 'profile' | 'musician' | 'presenter'
  | 'conductor' | 'live' | 'tools' | 'ai' | 'security' | 'access' | 'admin';
      ...
      {!user ? (
        ...
        <>
          <header className="sticky top-0 z-10 backdrop-blur-xl bg-[rgba(10,10,15,0.72)] border-b border-[var(--glass-border)]">
            ...
            {/* Navigation 2.0 — four essentials up front, everything else
                lives in the glass overlay menu (no more crowded scroll row) */}
            <nav className="max-w-5xl mx-auto px-2 pb-2 nav-shell" aria-label="Main">
              {tabs.filter((tb) => ['library', 'song', 'setlist', 'live'].includes(tb.id)).map(({ id, icon, label }) => (
                <button
                  key={id}
                  data-nav
                  onClick={() => setTab(id)}
                  aria-current={tab === id ? 'page' : undefined}
                  className={`glass-btn text-sm whitespace-nowrap ${tab === id ? 'primary' : ''}`}
                >
          ...
          <main className="max-w-5xl mx-auto px-4 py-6 pb-24">
            {tab === 'library' && (editing !== null ? (
              ...
              <LibrarySection songs={songs} canEdit={effectiveRole !== 'viewer'}
            ...
            {tab === 'song' && (song
              ...
              : <div className="glass-card p-8 text-center text-muted">{t('emptySetlist')}</div>)}
            {tab === 'setlist' && (
