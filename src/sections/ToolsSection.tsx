...
import { analyzeSheetImage } from '../omniscore/omr/staffDetect';
...
export default function ToolsSection() {
  ...
  const scanSheet = async (file: File) => {
    ...
    try {
      const r = await analyzeSheetImage(url);
      setScanResult(
        r.systems.length > 0
          ? `✓ Found ${r.systems.length} staff system(s) — confidence ${(r.confidence * 100).toFixed(0)}%. ` +
            `Lines at: ${r.systems[0].lines.join(', ')}px. Symbol recognition arrives in the next phase.`
          : 'No staff lines found — try a flatter, clearer photo of the sheet music.');
    } catch {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass-card p-6">
        ...
        <div className="space-y-4">
          <div>
            <p className="text-sm font-semibold mb-2">📄 Scan sheet music (photo → staff detection)</p>
            <input
