            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <input ref={bgRef} type="file" accept="image/*,video/*" multiple hidden
                     onChange={async (e) => {
                       ...
                       e.target.value = '';
                       for (const f of files) setBgList(await addChurchBg(church.code, f));
                       if (files.length) setMsg(`🖼 ${files.length} background item(s) added — they'll fade in behind your church's app.`);
                     }} />
            </div>
            {bgList.length > 0 && (
