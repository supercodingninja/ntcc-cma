...
export default function SongViewSection({ song, onSection }: Props) {
  const { t } = useI18n();
  ...
  // so a musician on stage never touches the iPad mid-song.
  useEffect(() => {
    if (!scrolling) return;
    const id = window.setInterval(() => {
      scrollRef.current?.scrollBy({ top: speed, behavior: 'auto' });
    }, 40);
